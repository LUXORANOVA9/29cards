...
class HandEvaluator {
    ...
    static evaluate(cards) {
        ...
        const sortedCards = Card_1.CardUtils.sortByValue(cards);
     ...
     */
    static evaluateTrail(cards, sum) {
        const [a, b, c] = cards;
     ...
     * Examples: 2+3+4=9
     */
    static evaluateNine(cards, sum) {
        // STRICT CHECK: Sum must be exactly 9 (NO Modulo)
        if (sum === 9) {
            // Among Nines, higher visible card wins
            const values = cards.map(c => Card_1.CardUtils.getValue(c));
     ...
     */
    static evaluateHighCard(cards, sum) {
        const values = cards.map(c => Card_1.CardUtils.getValue(c));
        const sortedValues = [...values].sort((a, b) => b - a);
        return {
